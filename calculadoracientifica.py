...
#define o nome no topo do programa
root.title("Calculadora Científica")

#define as cores do background da calculadora - podendo escolher qual for melhor conveniente
root.configure(background='white')

#estabelece valores fixos para largura e altura, não podendo ser modificado
root.resizable(widht=False, height=False)

#define a geometria
root.geometry("480x568+450+90")

#insere os botões na calculadora e atua como um recipiente para os valores e operadores
calc = Frame(root)

#cria um padrão de botões para a grade
calc.grid()

#agora será criado a classe que irá definir todas as funções da calculadora científica
class Calc():
    def _init_(self):
        self.total=0
        self.current=''
...
#o código abaixo irá criar o display
txtDisplay = Entry(calc,
				font=('Helvetica', 20, #pode utilizar a font que lhe convier
						'bold'),
				bg='black',
				fg='white',
				bd=30,
...
#agora será criado um teclado numérico
#guarde todos os números em uma variável
numberpad="789456123"

#aqui será contado as linhas para inserir os botões na grade
i=0

#crie uma lista em branco para armazenar
#cada botão terá uma especificação própria
btn=[]

#j está no intervalo para inserir o botão nessa linha específica
for j in range(2, 5):

    #k está no intervalo para inserir o botão nessa coluna específica
    for k in range(3):
        ...
        #estabelece botões em linha e coluna, além de separá-los com preenchimento de 1 unidade
        btn[i].grid(row=j, column=k, pady=1)

        #define o número como um símbolo neste botão
        btn[i]["Command"]=lambda x=numberpad[i]:added_value.numberEnter(x)
        i+=1

#agora será definido o local de todos os botões e operadores em suas respectivas posições na grade
...
#agora será criado um menu para a calculadora
#utilize a função askyeno para parar/continuar a execução do programa
def iExit():
    iExit=tkinter.messagebox.askyesno("Calculadora Científica", "Você deseja sair?")
    if iExit>0:
        root.destroy()
        return
...
filemenu.add_command(labe="Exit", command=iExit)
